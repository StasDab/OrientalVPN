...
- Оплата и пробный — **без выбора локации** (все серверы в одной подписке). Отдельная ссылка на один узел: меню «Выбрать сервер»; для узла в `VPN_NODES_JSON` задайте **`link_match`** (уникальная подстрока из vless-ссылки, например начало IP).
