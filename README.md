...
Бот выдаёт подписку VPN через Marzban API: пробный период, оплата (Telegram Payments или ЮKassa), **внутренний баланс** (пополнение произвольной суммой + списание при покупке тарифа), шлюз `/sub/{token}` с лимитом клиентов (по умолчанию **по IP**, чтобы Happ не ловил 403 из‑за смены `User-Agent`; строгий режим IP+UA — `SUBSCRIPTION_GATE_FINGERPRINT_USE_UA=true`).
...
| `app/config.py` | Настройки: читает **`.env`** в корне репо и **`app/.env`** (второй переопределяет совпадающие ключи). Массив нод при необходимости — в **`VPN_NODES_JSON_FILE`**. |
...
Миграция **`0006_sub_gate_ip_fp`** один раз очищает **`subscription_devices`**: старые отпечатки считались как IP+User-Agent; без очистки после смены логики на IP все запросы выглядели бы как новые устройства.
