...
- Оплата и пробный — **без выбора локации**: одна ссылка подписки на все серверы в клиенте; при включённом шлюзе (`SUBSCRIPTION_GATE_PUBLIC_BASE`) ссылка идёт через `/sub/{token}` с лимитом устройств.
